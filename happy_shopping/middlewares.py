...
class ReturnCookieJarCookiesMiddleware(CookiesMiddleware):
    ...
    def process_response(self, request, response, spider):
        ...
        if meta_cookie_jar_key in request.meta or (hasattr(spider,'trace_cookie_jar') and spider.trace_cookie_jar):
            cookiejarkey = request.meta.get("cookiejar")
            jar = self.jars[cookiejarkey]
            request.meta[meta_cookie_jar_key] = jar.jar
        if meta_cookie_map_key in request.meta or  (hasattr(spider,'trace_cookie_map') and spider.trace_cookie_map):
            cookiejarkey = request.meta.get("cookiejar")
            jar = self.jars[cookiejarkey]
